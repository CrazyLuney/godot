#include "enroth_physics_model_shape_3d.h"
...
struct _HeightmapSegmentCullParams {
	...
	const EnrothPhysicsModelShape3D *heightmap = nullptr;
...
_FORCE_INLINE_ bool _heightmap_chunk_cull_segment(_HeightmapSegmentCullParams &p_params, const _HeightmapGridCullState &p_state) {
	const EnrothPhysicsModelShape3D::Range &chunk = p_params.heightmap->_get_bounds_chunk(p_state.x, p_state.z);
...
void EnrothPhysicsModelShape3D::project_range(const Vector3 &p_normal, const Transform3D &p_transform, real_t &r_min, real_t &r_max) const {
	//not very useful, but not very used either
	p_transform.xform(get_aabb()).project_range_in_plane(Plane(p_normal), r_min, r_max);
}

Vector3 EnrothPhysicsModelShape3D::get_support(const Vector3 &p_normal) const {
	//not very useful, but not very used either
	return get_aabb().get_support(p_normal);
}

template <typename ProcessFunction>
bool EnrothPhysicsModelShape3D::_intersect_grid_segment(ProcessFunction &p_process, const Vector3 &p_begin, const Vector3 &p_end, int p_width, int p_depth, const Vector3 &offset, Vector3 &r_point, Vector3 &r_normal) const {
...
bool EnrothPhysicsModelShape3D::intersect_segment(const Vector3 &p_begin, const Vector3 &p_end, Vector3 &r_point, Vector3 &r_normal, int &r_face_index, bool p_hit_back_faces) const {
	if (heights.is_empty()) {
		return false;
	}
...
bool EnrothPhysicsModelShape3D::intersect_point(const Vector3 &p_point) const {
	return false;
}

Vector3 EnrothPhysicsModelShape3D::get_closest_point_to(const Vector3 &p_point) const {
	return Vector3();
}

void EnrothPhysicsModelShape3D::_get_cell(const Vector3 &p_point, int &r_x, int &r_y, int &r_z) const {
...
void EnrothPhysicsModelShape3D::cull(const AABB &p_local_aabb, QueryCallback p_callback, void *p_userdata, bool p_invert_backface_collision) const {
	if (heights.is_empty()) {
		return;
	}
...
Vector3 EnrothPhysicsModelShape3D::get_moment_of_inertia(real_t p_mass) const {
...
void EnrothPhysicsModelShape3D::_build_accelerator() {
	bounds_grid.clear();
...
void EnrothPhysicsModelShape3D::_setup(const Vector<real_t> &p_heights, int p_width, int p_depth, real_t p_cell_size, real_t p_min_height, real_t p_max_height) {
	heights = p_heights;
	width = p_width;
	depth = p_depth;
...
void EnrothPhysicsModelShape3D::set_data(const Variant &p_data) {
...
Variant EnrothPhysicsModelShape3D::get_data() const {
	Dictionary d;
	d["width"] = width;
	d["depth"] = depth;
...
EnrothPhysicsModelShape3D::EnrothPhysicsModelShape3D() {
}
